...
public sealed class BuildBootstrapPayloadUseCase
{
    ...
    public async Task<Result> ExecuteAsync(int eventId, int sinceSeq = 0)
    {
        ...
        var dto = new BootstrapPayloadDto(
            ...
            sinceSeq,
            projR.Value!.Select(p => new ProjectInfoDto(p.Id, p.Name, p.Description, p.Category, p.TeamMembers, p.SortOrder, p.ProjectCode, p.SequenceNumber, p.StandNumber, p.GithubLink, p.VideoLink, p.SpeechVideoLink, p.TechStack, p.Objetivos)).ToList(),
